...
public class Course {
    ...
    public static void setFirstRowFirstRow(File file) throws FileNotFoundException { // Read first row of file
        Scanner topRow = new Scanner(file);
        firstRow = Arrays.asList(topRow.nextLine().split(","));
        topRow.close();
    }

    public static void setHeader(List<String> firstRow){ // Exclude weights in final header
        header.add(0,"ID");
        header.add(1, "Name");
    ...
    public static void setWeights(List<String> firstRow, List<Double> weights) { // Store weights of different tests and assignments
